...
// Fetch the post ID from the URL and validate it
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $updated_post = mysqli_real_escape_string($con, $_POST['adminPostContent']);  // Sanitize the input
    ...
    if (mysqli_query($con, $update_query)) {
        header('Location: admin_dashboard.php'); // Redirect to the admin dashboard after editing
        exit();
    } else {
...
<body class="bg-gray-100 text-gray-900">
    ...
    <div class="max-w-6xl mx-auto my-8 bg-white p-6 rounded-lg shadow-lg">
        ...
        <form action="admin_edit.php?post_id=<?php echo $post_id; ?>" method="post" enctype="multipart/form-data">
            <textarea name="adminPostContent" class="w-full p-4 border border-gray-300 rounded-md" rows="6"><?php echo htmlspecialchars($post_content); ?></textarea>
            ...
            <div id="image-preview-container" class="mt-4">
                <?php if ($current_image): ?>
                    <img src="<?php echo $current_image; ?>" alt="Current Image" class="mt-4 w-32 h-32 object-cover rounded-md mx-auto">
                <?php endif; ?>
            </div>
